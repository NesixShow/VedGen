    ...
    public partial class OutPut : Form
    {
        ...
        private void OutPut_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    comboBoxGroup.Items.Add(row["TABLE_NAME"].ToString());
                }
                con.Close();
        ...
        {
            //я сразу решил закомментировать наперёд, дальше будут идти куски кода с paragraph это
            //это форматирование текста в word документе.
            //моменты с ref object это жертвы рефакторинга
            ...
            paragraph6.Range.Text = "Специальность/Профессия 09.02.07 Информационные системы и программирование " +
                "\nФорма обучения очная \nГруппа " + this.comboBoxGroup.Text + " Курс " 
            ...
            //следующий метод отвечает за форматирование таблицы, при экспорте увидите
            ...
            //вот до сюда идёт формирование таблицы
            ...
            //ВНИИИИМАААНИИИЕЕЕ то что ниже находится в комментарии, это жертва рефакторинга, в принципе я 
            //но при компиляции они автоматически создаются и добавляются, поэтому я их
            //и закомментировал, я честно не тупой и не забыл это удалить, так пока что надо
           ...
           /* ref object local17 = ref obj1;
            ...
            activeDocument.SaveAs(ref local17, ref local18, ref local19, ref local20, ref local21, ref local22, ref local23, 
                ref local24, ref local25, ref local26, ref local27, ref local28, ref local29, ref local30, ref local31, ref local32);
*/
        ...
        {
            string connectionString = "Server=(localdb)\\mssqllocaldb;Database=AutoVed;Trusted_Connection=True;";

            string selectCommandText = "SELECT FIO FROM " + this.comboBoxGroup.Text;
            ...
            using (SqlConnection selectConnection = new SqlConnection(connectionString))
            {
                ...
                this.dataGridView1.DataSource = (object)dataSet1.Tables[0];
            }
